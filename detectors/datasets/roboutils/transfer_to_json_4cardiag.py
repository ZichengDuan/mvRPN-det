...
def read_txt(left_right_dir):
    ...
    for k in range(0, 920):
        ...
        datas = []
        annotation = open("/home/dzc/Data/4cardiag/annotations/%d.json" % (k), 'w')
        for i in range(4):
            ...
            world_x, world_y, left_xmax, left_ymax, left_xmin, left_ymin, _, _ = [int(float(tmp)) for tmp in cont_left]
            right_xmax, right_ymax, right_xmin, right_ymin = [int(float(tmp)) for tmp in cont_right[2:6]]
            pID = i
            # ## 将角度转换为0-360度
            ...
            if np.all(lnew == 0) or np.sum(lnew) == 480 * 2 + 640 * 2 or lnarea / loarea < 0.3:
                lnew = (np.zeros(4) - 1).astype(np.int32)

            if np.all(rnew == 0) or np.sum(rnew) == 480 * 2 + 640 * 2 or rnarea / roarea < 0.3:
...
if __name__ == "__main__":
    read_txt("/home/dzc/Data/4cardiag/txt")