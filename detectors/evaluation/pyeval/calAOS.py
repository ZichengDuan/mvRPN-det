...
def CLEAR_MOD_HUN2(gt, det):
    ...
    for t in range(1, F + 1):
        ...
        frame_infolist = cal_frame_TPFP(39, gt_results, det_results)
...
def cal_frame_TPFP(dist_threshold, gt_res, pred_res):
    ...
    for i, pred in enumerate(pred_res):
        ...
        min_idx = -1
        cur_gt_ori = -1
        _, _, x_pred, y_pred, score, ori_pred = pred
        for j, gt in enumerate(gt_res):
            _, _, x_gt, y_gt, ori_gt = gt
...
def evaluateDetectionAPAOS(res_fpath, gt_fpath):
    ...
    for t in frames:
        ...
        idx = idxs[0]
        idx_len = len(idx)
        tmp_arr = np.zeros(shape=(idx_len, 5))
        ...
        tmp_arr[:, 4] = np.array([m for m in gtRaw[idx, -1]])
        ...
        idx = idxs[0]
        idx_len = len(idx)
        tmp_arr = np.zeros(shape=(idx_len, 6))
        tmp_arr[:, 0] = np.array([frame_ctr for n in range(idx_len)])
        tmp_arr[:, 1] = np.array([i for i in range(idx_len)])
        tmp_arr[:, 2] = np.array([j for j in detRaw[idx, 1]])
        tmp_arr[:, 3] = np.array([k for k in detRaw[idx, 2]])
        tmp_arr[:, 4] = np.array([m for m in detRaw[idx, -1]])
        tmp_arr[:, 5] = np.array([p for p in detRaw[idx, -2]])
...
if __name__ == "__main__":
    ...
    AP, AOS = evaluateDetectionAPAOS(res_fpath, gt_fpath)
    print("AP: %.2f" % (AP*100), "AOS: %.2f" % (AOS*100))