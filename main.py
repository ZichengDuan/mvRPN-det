...
def main(args):
    ...
    for epoch in tqdm.tqdm(range(1, args.epochs + 1)):
        if not args.resume:
            ...
            print('Testing...')
            model.load_state_dict(torch.load("%s/mvdet_rpn_%d.pth" % (Const.modelsavedir, 14)))
            roi_head.load_state_dict(torch.load("%s/roi_rpn_head_%d.pth" % (Const.modelsavedir, 14)))
            trainer.test(epoch, test_loader, writer)
            break
    writer.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr', type=float, default=0.000015, metavar='LR', help='learning rate (default: 0.1)')
