...
class ListDataset(Dataset):  # 多尺度载入模型
    ...
    def __getitem__(self, index):
        ...
        x1 = w_factor * np.clip((boxes[:, 1] - boxes[:, 3] / 2), a_min=0, a_max=None)
        y1 = h_factor * np.clip((boxes[:, 2] - boxes[:, 4] / 2), a_min=0, a_max=None)
        x2 = w_factor * np.clip((boxes[:, 1] + boxes[:, 3] / 2), a_min=0, a_max=None)
        y2 = h_factor * np.clip((boxes[:, 2] + boxes[:, 4] / 2), a_min=0, a_max=None)
...
class ListDataset_test(Dataset):  # 多尺度载入模型
    def __init__(self, list_path, img_size, augment=True, multiscale=True, normalized_labels=True):
        ...
        for i, readline in enumerate(self.img_files):
            ...
            llater = later.replace('.jpg', '.txt')
            lllater = llater.replace('\n', '')
            every_label_files = former + 'inilabels' + lllater
            self.label_files.append(every_label_files)
        self.img_size = img_size
        self.augment = augment
        self.multiscale = multiscale
