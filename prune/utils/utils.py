...
def write_results(prediction, confidence, num_classes, nms=True, nms_conf=0.4):
    ...
    for ind in range(batch_size):
        ...
        for cls in img_classes:
            ...
            class_mask_ind = torch.nonzero(cls_mask[:, -2]).szqueeze()
            ...
            if nms:
                ...
                for i in range(idx):
                    ...
                    # in the loop
                    try:
                        ious = bbox_iou(image_pred_class[i].unsqueeze(0), image_pred_class[i + 1:])  # 把iou大于某个阈值的都滤出
                    except ValueError:
                        break
...
def to_cpu(tensor):
    return tensor.detach().cpu()  # 分离到cpu上
...
def compute_ap(recall, precision):  # 输入的是recall和precision的curve
    ...
    for i in range(mpre.size - 1, 0, -1):  # range(start, stop[, step])
        mpre[i - 1] = np.maximum(mpre[i - 1], mpre[i])  # maximum：X和Y逐位进行比较,选择最大值.最少接受两个参数 只留下mpre的峰值 eg:[1,2,8,5,7]变为[8 8 8 7 7] [5,4,3,2,1]则不变
...
def build_targets(pred_boxes, pred_cls, target, anchors, ignore_thres):
    ...
    """tensor([[0.0000, 2.0000, 0.4012, 0.5266, 0.0367, 0.0281],
        [1.0000, 2.0000, 0.4129, 0.5215, 0.0258, 0.0211],
        [1.0000, 2.0000, 0.4711, 0.5434, 0.1047, 0.0867]])
    """
    ...
    # nG = pred_boxes.size(2)   # 相当于原图的缩放倍数为13

    nH = pred_boxes.size(2)
    nW = pred_boxes.size(3)
    # Output tensors [样本数，anchors数，grid宽，grid高]
    ...
    # target_boxes = target[:, 2:6] * nG    # target的0维是其对应图片的idx 1维是label的id
   ...
   # anchors*target  anchors与ground truth的iou  ious.shape:[anchor数(3), 一个batch内的target总数]  # 只考虑wh的iou
    ...
    b, target_labels = target[:, :2].long().t()  # target = [idx, labels, x, y, w, h]  所有行 前两列
    gx, gy = gxy.t()  # 转置
    gw, gh = gwh.t()
    ...
    for i, anchor_ious in enumerate(ious.t()):  # 图片索引和对应的anchors_iou
        ...
        # 区分预测的置信度值应该计算obj损失还是no_obj损失 ignore_thres只与置信度有关系
    ...
    iou_scores[b, best_n, gj, gi] = bbox_iou(pred_boxes[b, best_n, gj, gi], target_boxes, x1y1x2y2=False)  # 每个box的iou得分为iou值  考虑xywh的iou
