        ...
        if len(annotations):
            ...
            for pred_i, (pred_box, pred_label) in enumerate(zip(pred_boxes, pred_labels)):
                ...
                if iou >= iou_threshold and box_index not in detected_boxes:
                    true_positives[pred_i] = 1  # tp[i]值设为1
                    detected_boxes += [box_index]  # 大于iou阈值的列入detected_boxes中
...
def build_targets(pred_boxes, pred_cls, target, anchors, ignore_thres):
    ...
    # target_boxes = torch.stack((target[:, 3] * nH, target[:, 2] * nW, target[:, 5] * nH, target[:, 4] * nW), 0)  # target_boxes与target两个变量指向同一个元素,要改都会改
    # target_boxes1 = target[:, 2:6]      # target的1维是其对应图片的idx
    target_boxes = torch.stack((target[:, 2]*nW, target[:, 3]*nH, target[:, 4]*nW, target[:, 5]*nH), -1)
    ...
    print('obj_mask',obj_mask.sum())
    print('noobj_mask', noobj_mask.sum())
