...
from prune.models_lambda_resnet import *
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--pretrained_weights", '-pre', type=str, default="cfg/darknet53.conv.74", help="if specified starts from checkpoint model")
    ...
    parser.add_argument('--sparsity-regularization', '-sr', dest='sr', default=False, action='store_true',
    ...
    # Initiate model
    [_, _, resnum] = create_modules(parse_model_config(opt.model_def))
    createVar = dict()
    listTemp = range(resnum)
    for i, s in enumerate(listTemp):
        i = str(i)
        createVar['a' + i] = torch.nn.Parameter(torch.FloatTensor(1), requires_grad=True)
        createVar['a' + i].data.fill_(1)
    # print(createVar.keys())
    # print(createVar.values())
    alpha = opt.lr * opt.gamma

    model = Resnet(opt.model_def, createVar, device).to(device)  # model调用class Darknet类实例化 创建模型
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        num_workers=opt.n_cpu, pin_memory=True, drop_last=True)
    ...
    for epoch in range(0, opt.epochs):
        ...
        for subdivision_i, (imgs, targets) in enumerate(train_loader):
            ...
            loss, outputs, A = model(imgs, targets)  # 正向传播 loss += (label[k] - h) * (label[k] - h) / 2  此处的imgs和targets是通过dataloader函数载入的数据集的信息，对应Darknet_forward函数中的输入
            # print(A)
        ...
        res_param_txt_path = 'resnet_res_param_subdivision.txt'
        with open(res_param_txt_path, 'a+') as res_param:
            res_param.writelines('\n***********%d*************\n' % epoch)
        for i in range(len(A)):
            with open(res_param_txt_path, 'a+') as res_param:
                res_param.writelines(str(A['a' + str(i)]))
                res_param.writelines('\n')
