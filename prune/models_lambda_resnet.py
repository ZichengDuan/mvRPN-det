...
class Resnet(nn.Module):  # 继承nn.module类 # 在子类进行初始化时，也想继承父类的__init__()就通过super()实现
    ...
    def __init__(self, config_path, A, device):
        ...
        for i in range(len(self.A)):
            self.register_parameter("a" + str(i), self.A['a' + str(i)])  #　register_parameter(name, nn.Parameters)
        self.t = -1
        self.device = device
    ...
    def forward(self, x, targets=None):
        ...
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            ...
            if module_def["type"] in ["convolutional", "upsample", "maxpool", "avgpool"]:
                ...
                self.A['a' + str(self.t)] = self.A['a' + str(self.t)].to(self.device)
                x = self.A['a' + str(self.t)] * layer_outputs[-1] + layer_outputs[layer_i]
                print('a' + str(self.t), self.A['a' + str(self.t)])

            elif module_def["type"] == "softmax":  # [82, 94, 106] for yolov3
                x, loss = module[0](x, targets)  # module是nn.Sequential()，所以要取[0]
            layer_outputs.append(x)

        return x if targets is None else (loss, x, self.A)  # yolo层如果在训练时其output为其六个loss值；如不在训练则为其pred_box、conf、cls
